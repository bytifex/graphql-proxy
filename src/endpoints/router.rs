...
use config::{
    CONTROL_GRAPHQL_ENDPOINT, CONTROL_GRAPHQL_WS_ENDPOINT, PROXIED_GRAPHQL_ENDPOINT,
...
mod config {
    pub const CONTROL_GRAPHQL_ENDPOINT: &str = "/control-api/graphql";
    pub const CONTROL_GRAPHQL_WS_ENDPOINT: &str = "/control-api/graphql-ws";
...
pub fn routes(state: AppState, schema: Schema<Query, Mutation, Subscription>) -> Router {
    ...
    let control_graphql_routes = Router::new()
        .route(
            "/control-graphiql",
            get(|| graphiql(CONTROL_GRAPHQL_ENDPOINT, CONTROL_GRAPHQL_WS_ENDPOINT)),
        )
        .route_service(
            CONTROL_GRAPHQL_WS_ENDPOINT,
            GraphQLSubscription::new(schema.clone()),
        )
        .route(
            CONTROL_GRAPHQL_ENDPOINT,
